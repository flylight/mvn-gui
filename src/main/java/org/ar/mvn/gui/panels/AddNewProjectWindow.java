...
public class AddNewProjectWindow extends JDialog {
  ...
  public AddNewProjectWindow() {
    setTitle(LocaleUtil.getWord("ADD_NEW_PROJECT"));
    setSize(400, 170);
  ...
  private JPanel generateActionPanel() {
    ...
    //
    JButton add = new JButton(LocaleUtil.getWord("ADD"), Sources.ADD_SMALL_IMAGE);
      ...
      public void actionPerformed(ActionEvent e) {
        if (projectName.getText().isEmpty() || projectPath.getText().isEmpty()) {
          DialogMessagesUtil.showErrorMessage(AddNewProjectWindow.this,
              LocaleUtil.getWord("PROJECT_NAME_AND_PATH_CAN_NOT_BE_EMPTY"));
        } else {
          if (VerificationUtil.isThisFolderContainsActiveMavenProject(projectPath.getText())) {
            ...
            DialogMessagesUtil.showWarningMessage(AddNewProjectWindow.this, projectPath.getText()
                + LocaleUtil.getWord("NOT_CONTAINS_MAVEN_PROJECT"));
          }
        }
      }
    });
  ...
  private JPanel generateProjectInfoPanel() {
    ...
    c.gridx = 0;
    c.gridy = 0;
    JLabel nameLabel = new JLabel(LocaleUtil.getWord("PROJECT_NAME"));
    ...
    c.gridy = 1;
    c.gridx = 0;
    panel.add(new JLabel(LocaleUtil.getWord("EMPTY")), c);

    // LABEL
    c.gridy = 2;
    JLabel pathLabel = new JLabel(LocaleUtil.getWord("PROJECT_PATH"));
    ...
    // BUTTON
    c.gridx = 2;
    JButton selectPath = new JButton(LocaleUtil.getWord("EXTRA"));
  ...
  private void showPathChooser() {
    String path = OSUtil.showPathChooser(LocaleUtil.getWord("SELECT_MAVEN_PROJECT_FOLDER"), this);
    if (!path.isEmpty()) {
