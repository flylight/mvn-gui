...
public class SettingsPanel extends JPanel {
  ...
  private void buildUI() {
    ...
    c.gridx = 0;
    //
    settings.add(new JLabel(LocaleUtil.getWord("SELECT_MAVEN_HOME_FOLDER")), c);
    //
    c.gridx = 1;
    //
    ...
    c.gridx = 2;
    //
    JButton selectMavenFolderBtn = new JButton(LocaleUtil.getWord("EXTRA"));
    ...
    selectMavenFolderBtn.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        String path = OSUtil.showPathChooser(LocaleUtil.getWord("SELECT_MAVEN_HOME_FOLDER"), SettingsPanel.this);
        if (!path.isEmpty()) {
  ...
  private Component getControlPanel() {
    ...
    JButton save = new JButton(LocaleUtil.getWord("SAVE"));
    save.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        if (checkMavenHomeFolder()) {
          ...
          s.setLocale((String) localeComboBox.getSelectedItem());
          if (ApplicationStateManager.INSTANCE().saveSettings(s)) {
            DialogMessagesUtil.showInformationMessage(SettingsPanel.this, LocaleUtil.getWord("SETTINGS_SAVED"));
          } else {
            DialogMessagesUtil.showErrorMessage(SettingsPanel.this,
                LocaleUtil.getWord("ERROR_HAPPEN_WHEN_SAVE_SETTINGS"));
          }
        }
      }
    });
  ...
  private boolean checkMavenHomeFolder() {
    if (mavenHome != null && !mavenHome.getText().isEmpty()) {
      if (VerificationUtil.checkMavenHome(mavenHome.getText())) {
        ...
        DialogMessagesUtil.showErrorMessage(this, LocaleUtil.getWord("THIS_FOLDER_DO_NOT_CONTAINS_MAVEN"));
      }
    } else {
      DialogMessagesUtil.showErrorMessage(this, LocaleUtil.getWord("MAVEN_HOME_FOLDER_CAN_NOT_BE_EMPTY"));
    }
    return false;
  }
}
